...
export default function Home() {
  ...
  useEffect(() => {
    cuentaApi.getSaldo().then(r => setSaldo(r.data)).catch(() => {});
  }, []);

  useEffect(() => {
    const token = localStorage.getItem('token') || localStorage.getItem('admin_token') || '';
    fetch(API + '/avisos', { headers: { 'Authorization': 'Bearer ' + token } })
      .then(r => r.json())
    ...
    <div style={{ padding: '24px 16px' }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: 'var(--color-text-muted)', marginTop: 4, fontSize: 14 }}>
          {empleado?.cargo} en {empleado?.area}
        </p>
      </div>
      ...
      {saldo && (
        <div className="card" style={{ marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 28, fontWeight: 600, marginTop: 4 }}>
            <span className="moneda">{Number(saldo.saldo).toLocaleString('es-AR')}</span>
          </p>
          <span className={`badge badge-${saldo.estado === 'al_dia' ? 'success' : 'warning'}`} style={{ marginTop: 8 }}>
            {saldo.estado === 'al_dia' ? 'Al día' : saldo.estado}
          </span>
        </div>
      )}
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {avisos.map(a => (
            <div key={a.id} style={{
              padding: '14px 16px',
              borderRadius: 10,
              border: `1px solid ${a.importante ? '#F5A623' : '#e5e5e5'}`,
              background: a.importante ? '#FFFBF0' : '#fff',
              marginBottom: 10
            }}>
              <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 4 }}>
                {a.importante && '⚠️ '}{a.titulo}
              </div>
              <div style={{ fontSize: 13, color: '#555', marginBottom: 6 }}>{a.contenido}</div>
              {a.url_adjunto && a.tipo_adjunto === 'imagen' && (
                <img src={`https://huklwvkrykemdqpglwzr.supabase.co/storage/v1/object/public/${a.url_adjunto}`} alt="adjunto" style={{ maxWidth: '100%', borderRadius: 8, marginTop: 4 }} />
              )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {accesos.map((a) => (
          <button key={a.path} onClick={() => navigate(a.path)}
            style={{ background: a.color, border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', padding: 16, textAlign: 'left', cursor: 'pointer' }}>
